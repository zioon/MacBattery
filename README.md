...
| 充电上限（暂停充电） | AppleSMC 充电抑制键 `CH0B` / `CH0C` | **需 root**（同一个 root helper） |
...
> **为什么需要 root helper**：macOS **没有公开 API 能停止充电**。实际可行的手段是写 AppleSMC 的
> 充电抑制键（`CH0B` / `CH0C`：`0x00` = 允许、`0x02` = 抑制），**需要 root 权限**，
...
接外电 + 功能开启 + 电量 **≥** 上限 → 暂停充电；电量 **≤** 上限 − 5% → 恢复充电。
其中任一不满足则不下发指令：
...
- **设置面板**：开关、上限滑块（右侧即当前上限数值）、以及一行为**实际生效状态**——
  已关闭 / 充电中（会在 80% 暂停）/ 已到上限（充电已暂停）/ 未接电源 / 未装 helper /
  helper 未在运行 / helper 版本过旧 / 本机不支持。
...
- **不会留哑火状态**：App 每 5 秒刷新一次指令；超过 30 秒没刷新（App 退出 / 崩溃）时，
  helper 会**复位为「允许充电」**。宁可功能失效，也不能出现「关掉挂件后电永远充不满」。
- **写什么不由指令文件决定**：`/tmp` 是全局可写的，任何本地用户都能伪造指令文件，因此
  helper 只接受白名单动作（inhibit / allow）、只写白名单内的键与取值（`CH0B`/`CH0C` × `0x00`/`0x02`），
  且只写 `dataSize == 1` 的键。指令文件里没有任何字段能表达「写别的键 / 别的值」。
- **写完读回校验**：部分机型固件会静默忽略写入，不校验就会向界面谎报「已限充」。
- **不认识的键不动**：读到的当前值不是已知的两种取值时跳过该键（不同机型对 `CH0B`/`CH0C`
  的解释并不统一）。一个可安全写入的键都没有时，界面显示不可用并给出原因（见下条）。
